...
const ProductCard = memo(({ item, onAddToCart, onViewDetails }: ProductCardProps) => {
  return (
    <div className="bg-white rounded-xl shadow-sm overflow-hidden hover:shadow-md transition-shadow duration-300">
      ...
      <div className="p-3 sm:p-4">
        <div className="flex justify-between items-start gap-2">
          <div>
            <h3 className="text-base sm:text-lg font-semibold text-gray-900 leading-tight line-clamp-2">{item.name}</h3>
        ...
        <div className="mt-2 sm:mt-3 flex items-center space-x-2">
          ...
          <span className="text-xs sm:text-sm text-gray-600">
            {item.available_units > 0 ? `${item.available_units} left` : 'Out of stock'}
          </span>
        </div>
      </div>
    </div>
