...
import { processPayment, updateInventory } from '../lib/api';
import { Package2, X, Minus, Plus, ChevronRight } from 'lucide-react';
import type { Database } from '../lib/database.types';

type CartItem = Database['public']['Tables']['inventory']['Row'] & { quantity: number };
...
const CheckoutModal = memo(({ items, onClose, onSuccess, onQuantityChange, onViewDetails }: CheckoutModalProps) => {
          ...
          {items.map((item) => (
            <div key={item.id} className="flex items-center space-x-3 sm:space-x-4">
              ...
              <div className="flex items-center space-x-2 sm:space-x-3 bg-gray-50 rounded-lg p-1">
                ...
                <button
                  onClick={() => onQuantityChange(item.id, Math.min(item.available_units, item.quantity + 1))}
                  className="p-1.5 sm:p-2 hover:bg-white rounded-md transition-colors"
                >
        ...
        <div className="sticky bottom-0 bg-white border-t px-4 sm:px-6 py-4 sm:py-6 space-y-4">
          <div className="space-y-2">
            ...
            <div className="flex justify-between font-bold text-lg sm:text-xl pt-2 border-t">
              <span>Total</span>
              <span className="text-green-600">₹{(grandTotal / 100).toFixed(2)}</span>
            </div>
          </div>
